...
function listR2Buckets() {
    try {
        const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
        const cmd = accountId
            ? `npx wrangler r2 bucket list --account-id=${accountId} --json`
...
function listKVNamespaces() {
    try {
        const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
        const cmd = accountId
            ? `npx wrangler kv namespace list --account-id=${accountId} --json`
...
function deleteD1(name) {
    const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
    const cmd = accountId
        ? `npx wrangler d1 delete ${name} --account-id=${accountId} -y`
...
function deleteR2(name) {
    const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
    const cmd = accountId
        ? `npx wrangler r2 bucket delete ${name} --account-id=${accountId}`
...
function deleteKV(id) {
    const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
    const cmd = accountId
        ? `npx wrangler kv namespace delete --namespace-id=${id} --account-id=${accountId}`

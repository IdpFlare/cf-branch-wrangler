...
function listD1Databases() {
    try {
        const cmd = 'npx wrangler d1 list --json';
...
function listR2Buckets() {
    try {
        const cmd = 'npx wrangler r2 bucket list --json';
        const output = execSync(cmd, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
        return JSON.parse(output);
    } catch (_) {
        return [];
...
function listKVNamespaces() {
    try {
        const cmd = 'npx wrangler kv namespace list --json';
        const output = execSync(cmd, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
        return JSON.parse(output);
    } catch (_) {
        return [];
...
function deleteD1(name) {
    const cmd = `npx wrangler d1 delete ${name} -y`;
...
function deleteR2(name) {
    const cmd = `npx wrangler r2 bucket delete ${name}`;
    execSync(cmd, { stdio: 'inherit' });
}
...
function deleteKV(id) {
    const cmd = `npx wrangler kv namespace delete --namespace-id=${id}`;
    execSync(cmd, { stdio: 'inherit' });
}

...
function findD1Database(name) {
  try {
    ...
    const baseCmd = accountId
      ? `npx wrangler d1 list --account-id=${accountId}`
...
function findR2Bucket(name) {
  try {
    const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
    const cmd = accountId
      ? `npx wrangler r2 bucket list --account-id=${accountId}`
...
function findKVNamespace(name) {
  try {
    const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
    const baseCmd = accountId
      ? `npx wrangler kv namespace list --account-id=${accountId}`
...
function provisionD1(binding, suffix, configFormat) {
  ...
  const tmpConfigName = isJsonc ? '.branch-wrangler.json' : '.branch-wrangler.toml';
...
function provisionR2(binding, suffix) {
  ...
  if (!exists) {
    ...
    try {
      const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
      const cmd = accountId
        ? `npx wrangler r2 bucket create ${bucketName} --account-id=${accountId}`
...
function provisionKV(binding, suffix) {
  ...
  if (!namespaceId) {
    ...
    try {
      const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
      const cmd = accountId
        ? `npx wrangler kv namespace create ${namespaceName} --account-id=${accountId}`

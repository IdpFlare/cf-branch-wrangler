...
function findD1Database(name) {
  try {
    const baseCmd = 'npx wrangler d1 list';
...
function findKVNamespace(name) {
  try {
    const baseCmd = 'npx wrangler kv namespace list';
...
function provisionD1(binding, suffix, configFormat) {
  ...
  if (!dbId) {
    ...
    const cmd = `npx wrangler d1 create ${dbName}`;
...
function provisionR2(binding, suffix) {
  ...
  if (!exists) {
    ...
    try {
      const cmd = `npx wrangler r2 bucket create ${bucketName}`;
...
function provisionKV(binding, suffix) {
  ...
  if (!namespaceId) {
    ...
    try {
      const cmd = `npx wrangler kv namespace create ${namespaceName}`;

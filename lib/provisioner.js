...
/**
 * Finds a D1 database by name in the wrangler list output
...
function findD1Database(name) {
  try {
    ...
    // If list fails, assume database doesn't exist
    return null;
  }
}
...
function findR2Bucket(name) {
  try {
    ...
    const output = execSync(cmd, { encoding: 'utf-8' });
    // Match exact bucket name in quotes: "bucket-name"
...
function findKVNamespace(name) {
  try {
    ...
    for (const line of lines) {
      // Format: id="namespace-id" title="namespace-name"
      const idMatch = line.match(/id="([a-f0-9]+)"/);
      const titleMatch = line.match(/title="([^"]+)"/);
      if (idMatch && titleMatch && titleMatch[1] === name) {
        return idMatch[1];
      }
    }
    return null;
 ...
 */
function provisionD1(binding, suffix) {
  ...
  if (!dbId) {
    ...
    // Re-fetch to get the ID after creation
  ...
  if (fs.existsSync(seedFile)) {
    ...
    try {
      execSync(`npx wrangler d1 execute ${dbName} --remote --file=${seedFile}`, {
        stdio: 'inherit'
      });
    } catch (error) {
 ...
 */
function provisionAll(bindings, suffix) {
  const result = {
    d1: [],
    r2: [],
    kv: []
  ...
  for (const binding of bindings.d1) {
    result.d1.push(provisionD1(binding, suffix));
  }
